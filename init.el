...
(when (equal window-system 'ns)
  (use-package exec-path-from-shell
    ...
    (dolist (var '("DROPBOX" "BIBINPUTS" "BSTINPUTS" "GITHUB_TOKEN"
		   "OPENAI_API_KEY" "ANTHROPIC_API_KEY" "INRAE_API_KEY" "ALBERT_API_KEY" "OLLAMA_API_BASE"))
...
Also refresh the dirvish WINDOW that owns the active minibuffer."
    (if (and (minibuffer-window-active-p (frame-selected-window))
             (eq window (minibuffer-selected-window)))
        (dirvish--redisplay)
      (funcall orig-fn window)))
  (advice-add 'dirvish-pre-redisplay-h :around #'my/dirvish-render-in-minibuffer-a)
  :bind
  (("C-x D" . dirvish)
...
(use-package outline
  ...
  :custom
  (outline-minor-mode-cycle t) ; Use TAB on heading to cycle
...
(use-package corfu
  ...
  :config
  ;; Force corfu to use the default font to prevent the use of a variable pitch font with mixed-pitch
  (dolist (face '(corfu-default
                  corfu-current
                  corfu-annotations
                  corfu-deprecated))
...
(use-package forge
  ...
  :ensure t
  :hook
  ;; ~forge-post-mode~ derives from ~markdown-mode~, which triggers formatting
  ;; with ~apheleia~ but, since the post is immediately sent to the forge, the
  ;; formating leads to conflicts. Better to deactivate it.
  (forge-post-mode . (lambda () (apheleia-mode -1))))

(add-hook 'after-save-hook
          #'executable-make-buffer-file-executable-if-script-p)

(defconst my-proofread-instruction
...
Never replace a backslash followed by a percentage sign with just a percentage sign."
    "Instruction used by IA to proofread text.")

  (defconst my-copyedit-instruction
    "Help me copyedit this text. Reorganize sentences for better flow and clarity without altering the original meaning.
Detect the language of the text and respect it in the output.
If the text is in English, assume that it is in American English, unless indicated otherwise.
The aim of this text is for academic publication in the field of economics.
Output just the text without any intro, comments, or explanations.
...
Avoid using unicode for en dashes and em dashes, using '--' and '---' respectively.
Never replace a backslash followed by a percentage sign with just a percentage sign."
    "Instruction used by IA to copyedit text.")
...
If the original code was indented, preserve the same amount of spacing in your response:"
  ...
  (defun chatgpt-shell-copyedit-paragraph-or-region ()
    "Copyedit text from region or current paragraph using ChatGPT.

This function uses the existing proofreading capability but adjusts the prompt for copyediting tasks targeted at academic publications in economics."
    (interactive)
    (let ((chatgpt-shell-prompt-header-proofread-region
	   my-copyedit-instruction))
  ...
  (defvar-keymap chatgpt-shell-prefix-map
    ...
    "e" '("copyedit"              . chatgpt-shell-copy-edit-paragraph-or-region)
...
(use-package gptel
  ...
  ;; Chatbot INRAE
   (gptel-make-openai "ARGO"
    ...
    :stream t
    :models '("chat-gpt-oss-120b" "assistant-inrae-expert" "chat-gpt-oss-20b" "assistant-inrae"))
  :custom
  ;; Chat UI options
...
pending rewrite."
  ...
  (defun gptel-proofread-paragraph-or-region ()
    ...
    (interactive)
    (gptel--rewrite-paragraph-or-region my-proofread-instruction))

  (defun gptel-copyedit-paragraph-or-region ()
    "Copyedit the active region or current paragraph with `gptel-rewrite'."
    (interactive)
    (gptel--rewrite-paragraph-or-region my-copyedit-instruction))
  :bind
   ("C-c b p"        . gptel-proofread-paragraph-or-region)
   ("C-c b e"        . gptel-copyedit-paragraph-or-region))
...
(use-package eca
  ...
  ;; Rewrite
  (eca-rewrite-finished-action 'merge)
  :config
  (defun my-eca-proofread ()
    "Proofread the current text with ECA."
    (interactive)
    (eca-rewrite my-proofread-instruction))
  (defun my-eca-copyedit ()
    "Copyedit the current text with ECA."
    (interactive)
    (eca-rewrite my-copyedit-instruction))
  (defvar-keymap eca-operation-map
    ...
    "S" '("Switch to chat"            . eca-switch-to-chat)
...
(use-package visual-fill-column
  ...
  (defun my-visual-fill ()
    ...
    ;; To maximize the eca-chat window, don't use visual-fill-column-mode
